...
Load the data
```{r}
data <- read.csv("/Users/irina/Downloads/data.csv")
...
## PMT accuracy vs DTI
```{r}
# Select DTI metrics and math performance variables
dti_metrics <- c("FA_lh_mlf", "FA_rh_mlf", "AD_lh_mlf", "AD_rh_mlf", "RD_lh_mlf", "RD_rh_mlf", "MD_lh_mlf", "MD_rh_mlf")

# Create a list of math metrics
math_metrics <- c("ADD1", "ADD2", "ADD3", "SUB1", "SUB2", "SUB3", "MUL1", "MUL2", "MUL3", "DIV1", "DIV2", "DIV3")

# Initialize matrices to store correlations and p-values
cor_matrix_acc <- matrix(NA, nrow = length(dti_metrics), ncol = length(math_metrics))
p_matrix_acc <- matrix(NA, nrow = length(dti_metrics), ncol = length(math_metrics))

# Calculate correlations and p-values
for (i in 1:length(dti_metrics)) {
    for (j in 1:length(math_metrics)) {
        cor_test <- cor.test(data[[dti_metrics[i]]], data[[math_metrics[j]]])
        cor_matrix_acc[i, j] <- cor_test$estimate
        p_matrix_acc[i, j] <- cor_test$p.value
    }
}

# Convert matrices to data frames
cor_df_acc <- as.data.frame(cor_matrix_acc)
colnames(cor_df_acc) <- math_metrics
rownames(cor_df_acc) <- dti_metrics

p_df_acc <- as.data.frame(p_matrix_acc)
colnames(p_df_acc) <- math_metrics
rownames(p_df_acc) <- dti_metrics

cor_df_acc
p_df_acc

# Export
write.csv(cor_df_acc, "correlation_matrix_acc.csv")
write.csv(p_df_acc, "p_values_acc.csv")

# Initialize data frames to store residuals
dti_residuals <- data.frame(matrix(ncol = length(dti_metrics), nrow = nrow(data)))
colnames(dti_residuals) <- dti_metrics

math_residuals <- data.frame(matrix(ncol = length(math_metrics), nrow = nrow(data)))
colnames(math_residuals) <- math_metrics

# Fit linear models and calculate residuals for DTI metrics
for (metric in dti_metrics) {
    model <- lm(data[[metric]] ~ data$age)
    dti_residuals[[metric]] <- residuals(model)
}

# Fit linear models and calculate residuals for math metrics
for (metric in math_metrics) {
    model <- lm(data[[metric]] ~ data$age)
    math_residuals[[metric]] <- residuals(model)
}

# Initialize matrices to store correlations and p-values for residuals
cor_residuals_matrix_acc <- matrix(NA, nrow = length(dti_metrics), ncol = length(math_metrics))
p_residuals_matrix_acc <- matrix(NA, nrow = length(dti_metrics), ncol = length(math_metrics))

# Calculate correlations and p-values for residuals
for (i in 1:length(dti_metrics)) {
    for (j in 1:length(math_metrics)) {
        cor_test <- cor.test(dti_residuals[[dti_metrics[i]]], math_residuals[[math_metrics[j]]])
        cor_residuals_matrix_acc[i, j] <- cor_test$estimate
        p_residuals_matrix_acc[i, j] <- cor_test$p.value
    }
}

# Convert matrices to data frames
cor_residuals_df_acc <- as.data.frame(cor_residuals_matrix_acc)
colnames(cor_residuals_df_acc) <- math_metrics
rownames(cor_residuals_df_acc) <- dti_metrics

p_residuals_df_acc <- as.data.frame(p_residuals_matrix_acc)
colnames(p_residuals_df_acc) <- math_metrics
rownames(p_residuals_df_acc) <- dti_metrics

# Export
write.csv(cor_residuals_df_acc, "correlation_residuals_matrix_acc.csv")
write.csv(p_residuals_df_acc, "p_residuals_values_acc.csv")

cor_residuals_df_acc
p_residuals_df_acc
```
...
# Create a list of math metrics
math_metrics <- c("RT_ADD1", "RT_ADD2", "RT_ADD3", "RT_SUB1", "RT_SUB2", "RT_SUB3", "RT_MUL1", "RT_MUL2",
                    "RT_MUL3", "RT_DIV1", "RT_DIV2", "RT_DIV3")

# Initialize matrices to store correlations and p-values
cor_matrix_rt <- matrix(NA, nrow = length(dti_metrics), ncol = length(math_metrics))
p_matrix_rt <- matrix(NA, nrow = length(dti_metrics), ncol = length(math_metrics))

# Calculate correlations and p-values
for (i in 1:length(dti_metrics)) {
    for (j in 1:length(math_metrics)) {
        cor_test <- cor.test(data[[dti_metrics[i]]], data[[math_metrics[j]]])
        cor_matrix_rt[i, j] <- cor_test$estimate
        p_matrix_rt[i, j] <- cor_test$p.value
    }
}

# Convert matrices to data frames
cor_df_rt <- as.data.frame(cor_matrix_rt)
colnames(cor_df_rt) <- math_metrics
rownames(cor_df_rt) <- dti_metrics

p_df_rt <- as.data.frame(p_matrix_rt)
colnames(p_df_rt) <- math_metrics
rownames(p_df_rt) <- dti_metrics

cor_df_rt
p_df_rt

# Export the full correlation matrix as a CSV file
write.csv(cor_df_rt, "correlation_matrix_rt.csv")
write.csv(p_df_rt, "p_values_rt.csv")
...
# Initialize matrices to store correlations and p-values for residuals
cor_residuals_matrix_rt <- matrix(NA, nrow = length(dti_metrics), ncol = length(math_metrics))
p_residuals_matrix_rt <- matrix(NA, nrow = length(dti_metrics), ncol = length(math_metrics))

# Calculate correlations and p-values for residuals
for (i in 1:length(dti_metrics)) {
    for (j in 1:length(math_metrics)) {
        cor_test <- cor.test(dti_residuals[[dti_metrics[i]]], math_residuals[[math_metrics[j]]])
        cor_residuals_matrix_rt[i, j] <- cor_test$estimate
        p_residuals_matrix_rt[i, j] <- cor_test$p.value
    }
}

# Convert matrices to data frames
cor_residuals_df_rt <- as.data.frame(cor_residuals_matrix_rt)
colnames(cor_residuals_df_rt) <- math_metrics
rownames(cor_residuals_df_rt) <- dti_metrics

p_residuals_df_rt <- as.data.frame(p_residuals_matrix_rt)
colnames(p_residuals_df_rt) <- math_metrics
rownames(p_residuals_df_rt) <- dti_metrics

# Export the full correlation matrix of residuals as a CSV file
write.csv(cor_residuals_df_rt, "correlation_residuals_matrix_rt.csv")
write.csv(p_residuals_df_rt, "p_residuals_values_rt.csv")
