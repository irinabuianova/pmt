...
mean_sd_df <- data.frame(
  Variable = variables,
  Mean = sapply(variables, function(x) mean(data[[x]], na.rm = TRUE)),
  SD = sapply(variables, function(x) sd(data[[x]], na.rm = TRUE))
)

# Save the results
#write.csv(mean_sd_df, "mean_sd_results.csv", row.names = FALSE)
#write_xlsx(mean_sd_df, "mean_sd_results.xlsx")
print("Mean and standard deviation for each variable have been saved to 'mean_sd_results.csv' and 'mean_sd_results.xlsx'.")
print(mean_sd_df)
```
